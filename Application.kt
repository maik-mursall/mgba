...
private class RelayCoordinator {
    ...
    private suspend fun handleTransferStartLocked(session: ClientSession, payload: ByteArray) {
        ...
        stateLock.withLock {
            ...
            val modeMismatches = expectedIds
                .mapNotNull { id ->
                    val client = clients[id] ?: return@mapNotNull null
                    if (client.mode == mode) {
                        null
                    } else {
                        id to client.mode
            ...
            if (modeMismatches.isNotEmpty()) {
                val mismatchDetails = modeMismatches
                    .sortedBy { it.first }
                    .joinToString(", ") { "${it.first}:${it.second}" }
                logDebug(
                    "Ignoring TRANSFER_START from player ${session.id}: mode mismatch " +
                        "(startMode=$mode expected=[$mismatchDetails])"
                )
                return
            }
